    ...
    navigate("/detalles-compra", {
      state: {
        ...
        total: carrito.reduce((acc, item) => 
          acc + (parseFloat(item.producto.precio) * item.cantidad), 0)
      }
    });
  };
                      ...
                      <div key={index} className="producto-carrito">
                        <span>
                          <strong>
                            ...
                            S/. {parseFloat(item.producto.precio).toFixed(2)} ({item.cantidad})
                          </strong>
                          <br />
                          <strong>
                            S/. {(item.cantidad * parseFloat(item.producto.precio)).toFixed(2)}
                          </strong>
                        </span>
                        <button
                          className="btn-delet"
            ...
            {productosPaginados.map((producto) => (
              <div className="producto col-md-3 m-4 text-center" key={producto.id_producto}>
                ...
                <div className="d-flex justify-content-center align-items-center mb-2">
                  <label htmlFor={`cantidad-${producto.id_producto}`} className="me-2">
                    Cantidad:
                  </label>
                  <input
                    ...
                    onChange={(e) => manejarCambioCantidad(producto.id_producto, parseInt(e.target.value))}
                    style={{ width: "60px", textAlign: "center" }}
                  />
                </div>
                <button
                  className="btn-add"
                  onClick={() => agregarACarrito(producto, cantidades[producto.id_producto] || 1)}
                >
                  Añadir al Carrito
                </button>
              </div>
